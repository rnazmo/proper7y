...
- [x] Zsh フィールドで表示される値の改善を検討
  - 現在の出力はこう：`ZSH VERSION   : 5.9 (x86_64-pc-linux-gnu)`
  - `5.9` で良い気がする。`x86_64-pc-linux-gnu` はあまり意味のある情報ではないのでは？
  - Zsh のビルドターゲットを表示しても、ユーザーにとって有用な情報になるとは限らない
...
- [x] `print_chassis()` から仮想化判定ロジックを分離することを検討する
...
- [x] `make bump-project` で次のバージョンを入力する際に、現在のバージョンも表示する
...
- [x] `identify_current_shell_id()` の実装を見直す
  ...
  - 移植性の問題：ps のオプションは macOS と Linux で挙動が異なる（-c フラグの有無など）
    - `ps -cp "$PPID" -o command=""` の `-c` フラグは macOS (BSD ps) では「コマンド名のみ表示（パスなし）」という意味だが、
      Linux (GNU ps) には `-c` フラグが存在せず動作が異なる
  - ps で親プロセスを辿る方法は、CI 環境・Docker・`make` 経由での実行など、実行コンテキストが変わると容易に壊れる
  - そもそも「実行中のシェル」を Bash 単体で確実に特定する、というのには無理がありそう（本当に？要調査。）
  - `$SHELL` 環境変数を使う方法を検討する（ただし「デフォルトシェル」と「実行中のシェル」が異なる場合があるため、そのトレードオフをコメントかADRに明記する）
    - そもそも、ここで表示したい・すべきなのは「デフォルトシェル」なのか「実行中のシェル」なのかの検討から。
    - → proper7y において必要なのは「その proper7y コマンドが実行されたシェル」（つまり「実行中のシェル」）では？
    - その出力の再現性を高めるために「その proper7y コマンドが実行されたシェル」を表示しているわけで、
      妥協してデフォルトシェルを表示するくらいならば、シェルを表示は不要では？
    - いや、デフォルトシェルの情報であったとしても、「マシンの情報」としては一応有用では？
    - 「ログインシェル（デフォルトシェル）」を表示するかどうかは一旦ｎおいておいて、
      「proper7y を実行した（起動した）シェル」については、正常に取得できる（コントロールできている範囲である）ならば表示し、
      無理そうならば表示を諦めて項目ごと表示から消す（または大体としてログインシェルを表示する）みたいなのはどう？
  - 早めの対応が必要だが、検討事項が多くて対応が難しそう
  - **この検討は ADR で行うべき**
  - **この関数はスクリプトの中核機能に近いため、早急に行うべき**
- [ ] グローバル変数への依存を減らす（`common.bash` のグローバル変数設計の再考）
  - **背景・動機**
    - `common.bash` の関数群がグローバル変数に強く依存しており、「誰がいつ書き換えるか」の追跡が困難
    - バグの源泉になる予感がある。また、テストを書くための前提として設計を直したい
    - 学習目的のリファクタリングとして取り組む（ある程度大きな変更を許容する）
  - **マイルストーンの位置づけ（~~検討済み~~ 検討中）**
    - v0.11.0（テスト重点）の**前**に着手するのが望ましい順序
    - 理由：「テストを書きやすくするために設計を直す」という順序が自然であり、設計を直してからテストを書く方が無駄がない
    - ただし「何をテストしたいか」がv0.11.0で明確になるまでは、大きなリファクタリングには着手しない
    - 現時点でやること：小さく完結する改善（後述）のみ先行して行う
    - 追記：v0.11.0 で行う『テスト戦略の再設計」の前に、この「グローバル変数周りの再設計」を行うのは難しいかも。
      - それらは密接に絡み合っており、ある程度並行して検討する必要がありそう
      - 一方で両者（テスト戦略設計とグローバル変数設計）のそれぞれにおいて、「小さく簡潔する改善」があるので、
        まずはそれぞれでそういう部分に手を付けるのが良さそう
      - そのうえで残った大きな部分に関してを、v0.11.0 の改善事項として行う？
        （そうするとv0.11.0 のテーマを「テスト戦略の再設計」から変える必要ありそう）
  - **問題の本体（検討済み）**
    - グローバル変数を「使っていること」自体は Bash では一般的であり問題ではない
    - 本当に厄介なのは「書き換えが起きる変数」と「書き換えを引き起こす関数の連鎖」
    - 具体的には `SHELLCHECK_BINARY_VERSION` / `SHFMT_BINARY_VERSION` と、それを書き換える `_compose_*` / `_recompose_*` / `install_*` / `reinstall_*` の連鎖が「誰がいつ書き換えるか追えない」という問題の本体
  - **グローバル変数の再分類（検討済み）**
    - 分類A（設定値）: 起動時に決まり以後変わらない。`PROJECT_ROOT`, `SHELLCHECK_CMD_PATH` など
    - 分類B（ユーザー設定値）: `common.bash` に書かれた期待バージョン。`SHELLCHECK_CURRENT_VERSION` など。書き換えは `overwrite_version_number_variable` 経由のみ
    - 分類C（実行時取得値）: バイナリを実行して得られる実際のバージョン。`SHELLCHECK_BINARY_VERSION` など。書き換えが追いにくい主因
    - 分類D（一時的な中間値）: 関数内のローカル変数として扱うべきもの
  - **現時点でやること（小さく完結する改善）**
    - `common.bash` の変数定義部分に上記の分類（A〜D）と「誰が書き換えていいか」のルールをコメントで明記する
    - コードは変えない。把握負荷を下げ、v0.11.0 での設計議論の足がかりにする
    - 加えて、影響範囲が本当に狭い場合に限り、分類Cの改善（`get_shellcheck_binary_version()` など「値を返す関数」化）も検討する
  - **将来の実装方針（v0.11.0でテスト対象が明確になってから着手？）**
    - 基本方針：「関数はグローバル変数を読み書きしない。値は引数で受け取り、結果は `echo` で返す。グローバル変数の更新は呼び出し側の責任とする」
    - ファイル分割はしない（`source` が煩雑になるため）。基本1ファイル維持
    - ツールの汎用化（shellcheck/shfmt を抽象化）は（現時点では）しない。2ツール専用のままシンプルに保つ
    - 副作用の連鎖（インストール処理など）はルールを完全には満たせないケースがある。その場合は「関数がグローバル変数を書き換えることをコメントで明示する」という妥協点を設ける
  - **将来の実装順序（検討中）**
    - Step1: 分類Cの改善（`get_shellcheck_binary_version()` など「値を返す関数」化）→ 影響範囲が小さく始めやすい
    - Step2: 分類Bの改善（`bump_shellcheck_version()` などの引数化）→ 中程度の影響範囲
    - Step3: 分類Aの改善（`install_shellcheck()` などの引数化）→ 最も影響範囲が広い
    - 分類Bの改善は分類Aに依存している可能性あり。要調査。
  - **Bashの制約として把握済みの問題点**
    - Bashには複数の戻り値がないため、複数の値を返したい場合は工夫が必要（区切り文字での連結・`declare -n` 名前参照など）
...
- [x] README.md に GIF 動画を追加する
  ...
  - [x] フェーズ2: 準備
    ...
    - [x] 録画シナリオを書く
    - [x] 試し録りして品質・操作感の確認と、シナリオファイルの調整をする
  - [x] フェーズ3: 本番作業
    - [x] 本番録画してGIFを生成する
...
- [x] このプロジェクトが対応する環境・対象とするソフトウェアのドキュメント管理方法を再検討
- [x] README.md のコーディング規約を更新する
  ...
  - アイデアメモ：例えば、次のような内容を書きたい：
    ...
    - インデントはスペース2つ
    - 関数名はスネークケース
- [x] README.md のコーディング規約を再考する
  - 具体的に何を追加すべきかは、まだ不明確
  - 現在の規約セクションを見直し、不足している項目（例: シェルスクリプトの命名規則、関数のドキュメントコメント形式など）をリストアップする
  - それをもとにタスクを具体化する
  - ADR に書くべき？
    - これに関わる様々な規約(例えば、バージョニングやブランチ運用やコミットメッセージ、イシュー・プルリクエスト、ドキュメント、コードスタイルなど)について、どのような規約を採用して、そしてなぜそのような規約を制定したのか、ということを文書化しておきたい
      - ただし、コミットメッセージに関しては既にADR-009があるから不要かも?
    - README.md などに理由がちらほら書いてあったりするが、それらはもっとまとめるなりすべき
  - 不足・曖昧な部分：
    - 関数のドキュメントコメント形式 — 書き方のルールがない
    - ファイルヘッダコメント — 同上
    - `_` プレフィックスの命名規則 — 既にあるが「呼び出し禁止」の範囲が曖昧
    - 変数の大文字小文字 — 大文字と小文字が入り混じっている。グローバル変数は大文字にしたい。ローカル変数は未定
    - 日本語で書かれているセクションがある。英語で統一したい。
    - 全体的に書式が整っておらず、見づらい。
      - 見やすく簡潔にすべき。
      - 各規約について「なぜそのような規約を制定したのか」という理由も一言も書いておくと良さそうだが、
        一方で煩雑になる。難しい。
      - README.md や ADR-009 など、規約に関わる内容が分散しているのも問題。
        「なぜそのような規約を制定したのか」という理由も含めて、これらはもっとまとめるなりすべき。
...
- [x] `ADR.md` における ADR の並び順の方針をどこかに明記する
