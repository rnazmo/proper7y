...
- [ ] グローバル変数への依存を減らす（`common.bash` のグローバル変数設計の再考）
  ...
  - **設計方針（検討済み）**
    - 基本ルール：「関数はグローバル変数を読み書きしない。値は引数で受け取り、結果は `echo` で返す。グローバル変数の更新は呼び出し側の責任とする」
    - ファイル分割はしない（`source` が煩雑になるため）。基本1ファイル維持
    - ツールの汎用化（shellcheck/shfmt を抽象化）はしない。2ツール専用のままシンプルに保つ。ツールが増えたときに改めて検討
    - 副作用の連鎖（インストール処理など）はルールを完全には満たせないケースがある。その場合は「関数がグローバル変数を書き換えることは禁止するが、引数で受け取れない場合はコメントで明示する」という妥協点を設ける
  - **グローバル変数の再分類（検討済み）**
    - 分類A（設定値）: 起動時に決まり以後変わらない。`PROJECT_ROOT`, `SHELLCHECK_CMD_PATH` など。引数として渡す
    - 分類B（ユーザー設定値）: `common.bash` に書かれた期待バージョン。`SHELLCHECK_CURRENT_VERSION` など。読み取り専用として扱い、書き換えは `overwrite_version_number_variable` 経由のみで行われている。実行中のグローバル変数は書き換えない
    - 分類C（実行時取得値）: バイナリを実行して得られる実際のバージョン。`SHELLCHECK_BINARY_VERSION` など。「値を返す関数」に変えて、呼び出し側がグローバル変数に代入する責任を持つ
    - 分類D（一時的な中間値）: 関数内のローカル変数として扱う
  - **実装順序（検討中）**
    ...
    - 分類 B の改善は、分類Aの改善に依存しているかも？その場合、先に分類 A をやる必要があるかも。要調査。
  - **Bashの制約として把握済みの問題点**
    - Bashには複数の戻り値がないため、複数の値を返したい場合は工夫が必要（区切り文字での連結・`declare -n` 名前参照など）
      - 無理やり行うかんじになるので、汚い書き方に書き方になりがち。できれば避けたい。
  ...
  - また、現状 `common.bash` の `initialize_global_variables()` が2回呼ばれるとエラーになる設計になっている（関数内で `readonly` 宣言している）ので、それも考慮すること。具体的には、冪等に動作するよう設計し直すことを検討
    - ただし、これはグローバル変数の再設計とは別のタスクかも。ただし、かなり密接に関わっている。
