...
### タスクの優先順位メモ (2026-04-27 時点)

各タスクの依存関係を踏まえ、以下の順番での進行を推奨。

また「ADRで方針を固める → 実装作業」という流れが基本となる。

1. 「環境依存フィールドのテスト設計を再考する」のADRを書く
  - ここの結論が「workaround解消」「レベル3強化」「ユニットテスト検討」の3タスクすべての前提になる
1. `run-integ-test.bash` のworkaround解消
1. integ-testのアサーションをレベル3に強化
1. 「ユニットテストを追加するか」のADRを書く
1. 「common.bash のグローバル変数設計を見直す」のADRを書く
  - テスト設計のADRと並行して検討？
  - 実装コストが大きい
...
- [ ] README.md に日本語の記述が混じっているのを英語に統一する
