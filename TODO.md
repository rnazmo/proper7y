...
- [ ] proper7y 内の関数のユニットテスト状況・可能性・副作用の有無などをまとめ、表などにして文書化しておく
  - 現時点での proper7y 内の関数の一覧を作り、表にしたい
  - ユニットテストに関する検討・判断を行う際の参考材料とするため
  - 例えば、以下のような項目を含めることを検討する
    - 関数名
    - 簡単な説明
    - ユニットテストの対象にできるか（Yes/No）
    - ユニットテストの対象にする場合の実装方針（例: `is_supported()` の場合は「引数と出力で完結しているので、そのままテスト可能」など）
    - ユニットテストの対象にしない場合の理由（例: `install_shellcheck()` の場合は「グローバル変数を書き換える副作用があるため、現状の設計ではテストが難しい」など）
    - 副作用の有無？
    - 既にユニットテストしているかどうか？
    - ユニットテストを追加する場合のコスト感（Low/Medium/High など）？
    - ユニットテストを追加する場合のリターン感（Low/Medium/High など）？
  - その表には作成日も書いておき、「この情報は現時点でのものであるから、
    必要に応じて内容を適宜更新して下さい」的なことを付記しておきたい。
    （常に最新版であることを保証するのではなく、参照する際にその情報が最新版かどうか確認し、
    必要に応じて記述を更新する、という形式を取りたい）
- [ ] ユニットテストの対象範囲を拡大を検討する（「副作用のある関数」について）
  - Ref: ADR-030
  - 副作用を分離したコード設計への書き直しも検討
  - 別タスク `common.bash のグローバル変数設計を見直す` とも関連するかも。
    その場合、並行して検討を進める必要がありそう。
  - **コストかなり高い**
...
- [ ] refactor: `proper7y` の `print_` 系関数から、ロジックを分離する
  - 現状では、`print_` 系関数がロジックも出力も担当しているため、ユニットテストが難しい状態になっている
  - リファクタリングやユニットテスト範囲の拡大の第一歩として、この変更は取り組みやすい
