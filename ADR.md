...
- **決定:**
  ...
  - **VHS のインストール管理:** `devel-tools/script/install-demo-tools.linux-x64.bash` として管理する
...
- **理由:**
  - **VHS を採用する理由:**
    - シナリオを `.tape` ファイルとしてバージョン管理できるため、将来の再録画が容易
...
- **影響:**
  ...
  - `devel-tools/script/install-demo-tools.linux-x64.bash` の作成は今後の作業として残る
  - GIF の生成と README.md への埋め込みは今後の作業として残る
