...
## ADR-037: `sed -i` のクロスプラットフォーム対応方針 — `sed_i()` ラッパー関数

- **日付:** 2026-06-11
- **状況:**
  - `overwrite_version_number_variable()` が `sed -i` を使用している。
  - GNU sed（Linux）は `sed -i 's/.../' file` で動作するが、BSD sed（macOS）は
    `sed -i '' 's/.../' file` と空文字の引数が必要であり、互換性がない。
  - 現状、この関数は `bump-project.linux-x64.bash`（Linux 専用）からのみ呼ばれるため
    実害はないが、プロジェクト全体として「暗黙に GNU sed を仮定している」状態であり、
    将来 macOS 対応や別の箇所での `sed -i` 使用時にバグの原因となりうる。

- **決定:**
  - `common.bash` に `sed_i()` ラッパー関数を追加する。
  - `overwrite_version_number_variable()` の `sed -i` 呼び出しを `sed_i` に置き換える。
  - 新たに `sed -i` が必要になった場合は、常に `sed_i` を使用する。

- **検討した代替案:**
  - **案1（却下）: 現状維持**
    - 問題の先送りに過ぎず、将来のバグを予防できない。
  - **案2（却下）: 全スクリプトで macOS 対応を徹底する方針を ADR のみで定める**
    - 人的ミスを防止できない。
  - **案3（採用）: `sed_i()` ラッパー関数を導入する**
    - 変更範囲が小さい。呼び出し元は意識せずに使える。
    - macOS 環境で `uname -s` を毎回呼ぶオーバーヘッドがあるが、バンプ作業中に
      数回呼ばれる程度で無視できる。
    - 将来、抜本的な対応（例: `gnu-sed` の brew install を必須にする等）に
      変更する場合も、この関数1箇所の修正で済む。

- **影響:**
  - Linux 環境での動作に変更はない。
  - macOS 環境で `uname -s` が1回余分に実行されるようになるが、無視できる範囲。
